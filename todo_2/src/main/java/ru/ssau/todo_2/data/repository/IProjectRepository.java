...
public interface IProjectRepository extends JpaRepository<Project, UUID>{
    ...
    @Query(value = "select P.id as ProjectId , count(T.id) as TasksNumber from project as P left join task as T on T.projectid = P.id group by P.id", nativeQuery = true)
